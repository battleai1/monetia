...
### Latest Updates
- ✅ **Intro Countdown Animation** (Стартовый отсчёт) - ПОЛНОСТЬЮ ПЕРЕДЕЛАНО:
  ...
  - **Параметры:**
    - Длительность тика (T): **700ms**
    - Перекрытие (overlap): **40%** (280ms)
    - Следующая цифра стартует через: **420ms** (T * 0.6)
  - **Ключевая анимация цифры:**
    - Старт: opacity 0, scale 0.7, blur 0
    - Пик (25%): opacity 1, scale 1.0, blur 0
    - Конец: opacity 0, scale 2.0, blur 10px
    - Easing: cubic-bezier(0.4, 0, 0.2, 1)
  - **Хэптик привязан к пику** (~35% от 700ms = 245ms):
    - Telegram HapticFeedback: medium для 5-2, heavy для 1
    - Фолбэк navigator.vibrate(30ms) только после user gesture
