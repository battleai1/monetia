...
- ✅ **Intro Countdown Animation** (Стартовый отсчёт) - ОБНОВЛЕНО:
  ...
  - **Эффект "лопающихся мыльных пузырей"** - цифры взрываются на весь экран: 
    - Появление: scale 0.5→1.0 (150ms)
    - Стоп: держится на scale 1.0 (300ms)
    - Взрыв: scale 1.0→1.2→8.0 с blur 0→2px→40px (750ms)
    - **Следующая цифра появляется на фоне** во время взрыва предыдущей
