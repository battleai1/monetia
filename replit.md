...
- ✅ **Intro Countdown Animation** (Стартовый отсчёт) - ОБНОВЛЕНО:
  ...
  - **Эффект "лопающихся мыльных пузырей"** - цифры взрываются на весь экран: 
    - Появление: scale 0.5→1.0 (180ms, 0-15%)
    - Стоп: держится на scale 1.0 (360ms, 15-45%)
    - Рост: scale 1.0→1.2 (300ms, 45-70%)
    - Взрыв: scale 1.2→8.0 с blur 0→2px→40px (360ms, 70-100%)
    - **Следующая цифра появляется через 100ms после начала взрыва** (940ms от старта цифры)
